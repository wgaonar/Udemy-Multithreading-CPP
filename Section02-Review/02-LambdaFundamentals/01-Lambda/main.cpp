...
int main()
{
  ...
  */
  ...
  std::cout << "l1(3): " << l1(3) << std::endl << std::endl;  // 30

  // Capture a local variable for modifying it inside the lambda
  int m{10};
  std::cout << "m before lambda: " << m << std::endl; // 10

  auto l2 = [&m] (int arg) { return ++m * arg; };
  std::cout << "l2(3): " << l2(3) << std::endl ;   // 33
  std::cout << "m after lambda: " << m << std::endl << std::endl; // 11
}